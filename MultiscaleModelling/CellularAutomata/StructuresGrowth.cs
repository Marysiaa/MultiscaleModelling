...
namespace MultiscaleModelling.CellularAutomata
{
    public class StructuresGrowth
    {
        ...
        public StructuresGrowth(Random random, Scope baseScope)
        {
            this.random = random;
        ...
        {
            var reainingGrains = new List<Dictionary<Point, Grain>>();
            foreach (var point in grainPoints)
            {
                reainingGrains.Add(GetWholeGrain(point));
            }
            return reainingGrains;
        }
        ...
        private Scope GenerateSubstructure(List<Dictionary<Point, Grain>> remainingGrains, SimulationProperties properties)
        {
            ...
            var prevScope = StructureHelpers.InitStructure(properties, random, scope, remainingIds);
        ...
        {
            var prevScope = StructureHelpers.InitStructure(properties, random);
            var newScope = new Scope(prevScope.Width, prevScope.Height);
            // CA 
            while (newScope == null || !newScope.IsFull)
            {
